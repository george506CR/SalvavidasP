...
<body>
    ...
    <div class="container text-center my-auto">
        <div class="m-4 d-flex justify-content-center align-items-center">
            <!--Exportar Visitas-->
            <a href="dumpListaEntradasSalidas.php" target="_blank" style="color: #f6695e; ">
                <button type="button" class="btn_agregar">Exportar Entradas y Salidas</button>
            </a>
        </div>
    </div>
    <div class="container text-center my-auto">
        <div class="m-4 d-flex justify-content-center align-items-center">
            <?php
                echo "<table width=\"900px\" style=\"border-collapse: separate;\">
                    <tr>
                ...
                $sql = "SELECT es.etsId,es.etsFecha,p.psrNombre,p.psrPrimerApellido,p.psrSegundoApellido, es.etsHoraEntrada, es.etsHoraSalida
                FROM entradassalidas es
                JOIN funcionario f ON es.etsIdFuncionario=f.fncId
                JOIN persona p ON f.fncIdPersona=p.psrID";
                foreach ($db->query($sql) as $row) {
                    $etsHoraEntradaDB = $row["etsHoraEntrada"];
                    $etsHoraEntradaDBFormated = date('Y-m-d h:i:s A', strtotime($etsHoraEntradaDB));
                    $etsHoraSalidaDB = $row["etsHoraSalida"];
                    if($etsHoraSalidaDB == NULL){
                        $etsHoraSalidaDBFormated = '';
                    }else{
                        $etsHoraSalidaDBFormated = date('Y-m-d h:i:s A', strtotime($etsHoraSalidaDB));
                    }
                    echo "<tr>
                        <td style=\"border: 1px solid #ff7f50;\">".$row["etsFecha"]."</td>
                        <td style=\"border: 1px solid #ff7f50;\">".$row["psrNombre"]." ".$row["psrPrimerApellido"]." ".$row["psrSegundoApellido"]."</td>
                        <td style=\"border: 1px solid #ff7f50;\">".$etsHoraEntradaDBFormated."</td>
                        <td style=\"border: 1px solid #ff7f50;\">".$etsHoraSalidaDBFormated."</td>
                    </tr>";
                }
                echo "</table>";
            ?>
